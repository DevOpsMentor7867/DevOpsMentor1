...
export function useChatSocket(enabled = true) {
    ...
    if (!socketRef.current) {
      ...
      try {
        ...
        socketRef.current.on("connect", () => {
          console.log("Chat Socket connected successfully, ID:", socketRef.current.id)
          setIsConnected(true)
        ...
        socketRef.current.on("onlineUsers", (users) => {
          ...
          const filteredUsers = Array.isArray(users)
            ? users.filter((onlineUser) => onlineUser.username !== user.username)
            : []
  ...
  useEffect(() => {
    if (connectionAttempts > 0 && connectionAttempts <= 3 && !socketRef.current && enabled && user) {
      // Wait before trying again with different options
      const timer = setTimeout(() => {
        console.log(`Retrying connection with alternative approach #${connectionAttempts}`)
        ...
        try {
          ...
          if (socketRef.current) {
            console.log(`Alternative connection attempt #${connectionAttempts} created`)

            // Re-attach all event listeners
            socketRef.current.on("connect", () => {
              console.log("Chat Socket connected with alternative approach, ID:", socketRef.current.id)
              setIsConnected(true)
              setSocketId(socketRef.current.id)
              socketRef.current.emit("register", user.username)
              setConnectionAttempts(0)
            ...
            socketRef.current.on("connect_error", (error) => {
              console.error(`Alternative attempt #${connectionAttempts} connection error:`, error)
              setIsConnected(false)
          ...
          console.error(`Error in alternative connection attempt #${connectionAttempts}:`, error)
          socketRef.current = null
        }
